...
**Results.** Species-only RF achieved per-cohort mean LODO AUC 0.807 ± 0.065 and pooled AUC **0.781 (95% CI 0.757-0.805)**, modestly outperforming joint RF (0.756; **DeLong z = 3.35, p = 0.0008**) and joint XGBoost (0.766; z = 2.00, p = 0.046); the pooled ΔAUC is ≈0.02 and the DeLong signal is concentrated in the largest fold (YachidaS_2019, n_test = 508). Country-aware LODO removed population-level confounding: ThomasAM_2019_c AUC dropped from 0.998 to 0.836 when same-country YachidaS_2019 was excluded. Results were stable across five random seeds (0.810 ± 0.002), a 20-cell pathway-threshold grid (0.781-0.835, spread 0.055), demographic adjustment (0.800-0.814), and per-fold ComBat correction.

**Conclusions.** At current cross-cohort sample sizes, species-level features alone marginally outperform joint species-plus-pathway models on pooled CRC discrimination; the small pooled ΔAUC is driven primarily by one large fold, and pathway features add dimensionality without proportional signal. Formal classifier comparison and systematic robustness evaluation should be standard in metagenomic ML studies.
