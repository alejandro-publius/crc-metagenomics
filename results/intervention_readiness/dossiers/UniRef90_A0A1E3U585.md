# Uncharacterized protein

- **Candidate ID:** `UniRef90_A0A1E3U585`
- **Class:** precision_address_candidate
- **Current atlas status:** rejected_no_parent_independent_signal

## Why it is in the atlas

selected in 7 outer folds; 86% held-out direction consistency; median held-out AUC 0.551

## Evidence gates

| Gate | Current status |
|---|---|
| Human cross-population recurrence | internal_cross_fitted_pass |
| Signal beyond annotated parent species | not_passed |
| Taxon-resolved carrier | not_yet_assessed |
| Biological function or harmful-clade link | not_assessed_parent_gate_failed |
| Sequence conservation | not_yet_assessed |
| Specificity against protected organisms/human sequence | not_yet_assessed |
| Editing and delivery feasibility | platform_dependent_not_yet_assessed |
| External gene-level confirmation | not_yet_assessed |

## Claim boundary

The family recurs across cohorts but does not add enough held-out information beyond its annotated parent-species proxy.

## Address-resolution detail

did not pass: median held-out AUC gain 0.009; positive in 70% of evaluable folds

taxon-resolved carrier evidence has not been evaluated

## Experiment-enabling next evidence

Resolve every non-passing gate with versioned public data or a prespecified
laboratory experiment. This dossier must not be presented as a treatment
recommendation or institutional endorsement.
