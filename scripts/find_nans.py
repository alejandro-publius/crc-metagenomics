"""Scan the species, pathway, and metadata processed files for NaN entries.

Merges species + pathway + metadata on `sample_id` and prints any feature
columns containing NaN, plus the per-source NaN count (so the offending
file can be identified quickly). Run after any pipeline change that
touches preprocessing, filter_pathways, or merge_pathways.
"""
import pandas as pd
