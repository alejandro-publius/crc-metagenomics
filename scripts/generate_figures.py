"""Legacy convenience figures for early drafts of the manuscript.

Reads existing results CSVs (no model refits) and writes four PNGs into
``figures/``:

- ``fig1_lodo_auc.png``        — per-cohort grouped bars for species RF,
                                  joint RF, joint XGBoost LODO AUCs.
- ``fig2_shap_crc.png``        — top-15 species by mean |SHAP| for the
                                  species RF CRC-vs-control model.
- ``fig3_adenoma.png``         — within-cohort 5-fold adenoma AUCs from
                                  the deprecated ``train_adenoma.py``.
- ``fig4_external_validation.png`` — per-cohort + combined AUC for the
                                  external-validation sanity check.

The headline manuscript figures are generated by
``figure1_forest_plot.py`` and ``figure5_shap_three_panel.py``; this
script is kept for backward compatibility with older notebooks.
"""
