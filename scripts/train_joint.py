...
`tree_method='hist'`). A per-fold prevalence/mean pathway filter
(prevalence >=10%, mean >=1e-6 by default) is recomputed on
training-cohort samples only inside each fold; species features pass
through (the species filter is global, in `preprocessing.py`). Writes:
...
CLI flags:
    --prevalence-threshold FLOAT  (default 0.10)
    --mean-threshold FLOAT        (default 1e-6)
"""
import argparse
import os
import re
...
def main(argv=None):
    ...
    # Structural restriction (definitional, not data-derived): keep only
    # UNSTRATIFIED pathway columns -- i.e. those whose name does NOT
    # contain '|'. HUMAnN-stratified columns are encoded as
    # "PATHWAY|genus.species"; if we ever fed such a frame in (e.g. a
    # strain/gene matrix), the previous `'|' not in c` filter would have
    # silently stripped every feature and produced a species-only model
    # mislabelled as "joint". The assertion below makes that failure mode
    # loud rather than silent.
    pw_unstrat_cols = [c for c in pw_raw.columns
    ...
    # Loud failure if structural filter accidentally stripped everything
    # (e.g. an all-stratified frame was passed in). Without this guard
    # the joint model silently degenerates to species-only.
    n_pre_fold_features = X.shape[1]
    if n_pre_fold_features <= MIN_JOINT_FEATURES:
        raise ValueError(
            f"train_joint.py: pre-fold joint feature count is "
            f"{n_pre_fold_features} (<= {MIN_JOINT_FEATURES}). "
            f"Species kept: {len(sp_feat_cols)}, unstratified pathway "
            f"candidates kept: {len(pw_feat_cols)}. The structural "
            f"unstratified-only filter likely stripped every column "
            f"(all input columns contained '|'). Aborting rather than "
            f"silently training a species-only model labelled 'joint'."
        )
    ...
    def pathway_filter(X_train):
        ...
        keep_pw = [c for c in pw_feat_cols
                   if prev[c] >= prevalence_threshold
                   and ma[c] >= mean_threshold]
    ...
    def make_xgb():
        # tree_method='hist' keeps wall time per fold sane (~seconds, not
        # 30+ minutes) on wide feature matrices, which matters as soon as
        # we layer in stratified / gene / strain columns.
