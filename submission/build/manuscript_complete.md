...
Stratified taxon|pathway features were considered but excluded: they produce >4,000 highly redundant columns and did not improve AUC in pilot tests.
...
**Batch correction.** Per-fold ComBat (Johnson et al. 2007), as implemented in `combat.pycombat.pycombat`, was applied to species features within each LODO fold. ComBat was fit jointly on train and test feature matrices using only batch labels (`study_name`); class labels were never seen by ComBat, preserving LODO no-leakage while keeping train and test in the same corrected feature space.
...
## Why pathways do not help, despite biological plausibility

Microbial metabolic pathways implicated in CRC — choline trimethylamine-lyase, secondary bile-acid biosynthesis, polyamine metabolism, hydrogen sulfide production, butyrate fermentation — are biologically well-motivated and have been highlighted by prior cross-cohort studies (Thomas et al. 2019; Wirbel et al. 2019). Why, then, do they not improve a joint classifier?

The simplest explanation is dimensionality. Joining 402–406 per-fold pathway features to the 229 species features nearly triples the feature count without a proportional gain in discriminative signal. Under Random Forest with `max_features = 'sqrt'`, the probability that the most informative species are sampled at each split decreases as the candidate pool grows; under XGBoost with `colsample_bytree = 0.8`, a similar dilution occurs. The DeLong signal is driven primarily by the largest fold (YachidaS_2019, n_test = 508), where the species-only model reaches AUC 0.708 versus 0.669 (joint RF) and 0.694 (joint XGBoost) — exactly the fold where stable feature ranking matters most.

A complementary explanation is redundancy. Functional pathway abundance is heavily correlated with the taxa that encode the corresponding genes; the four oral pathobionts that top the species SHAP rankings (*Fusobacterium nucleatum*, *Parvimonas micra*, *Peptostreptococcus stomatis*, *Gemella morbillorum*) contribute to a wide set of pathways at the unstratified level, so much of the pathway "signal" is already captured by the species. The biologically-guided 84-pathway shortlist (mean LODO AUC 0.817 across cohorts; `results/bio_pathway_results.csv`) confirms this interpretation: hand-curated CRC-relevant pathways perform comparably to the species baseline rather than outperforming it.

This is consistent with the broader high-dimensional / low-sample-size literature (Bellman 1961; Trunk 1979): at n ≈ 1,300 across heterogeneous cohorts, parsimony wins. It also harmonises with the recent Piccinno et al. (2025) result of mean AUC ~0.85 across 18 cohorts and 3,741 metagenomes: with substantially more training data the noise contribution of additional features may diminish. Our finding should be read as a statement about the current sample-size regime, not as a categorical claim that functional features can never help.
...
1. Austin, G.I., Brown Kav, A., ElNaggar, S. et al. Processing-bias correction with DEBIAS-M improves cross-study generalization of microbiome-based prediction models. *Nat. Microbiol.* **10**, 897–911 (2025). https://doi.org/10.1038/s41564-025-01954-4

2. Bellman, R. *Adaptive Control Processes: A Guided Tour*. Princeton University Press (1961).

3. Chen, T. & Guestrin, C. XGBoost: a scalable tree boosting system. In *Proc. 22nd ACM SIGKDD International Conference on Knowledge Discovery and Data Mining* 785–794 (ACM, 2016). https://doi.org/10.1145/2939672.2939785

4. DeLong, E.R., DeLong, D.M. & Clarke-Pearson, D.L. Comparing the areas under two or more correlated receiver operating characteristic curves: a nonparametric approach. *Biometrics* **44**, 837–845 (1988). https://doi.org/10.2307/2531595

5. Franzosa, E.A., McIver, L.J., Rahnavard, G. et al. Species-level functional profiling of metagenomes and metatranscriptomes. *Nat. Methods* **15**, 962–968 (2018). https://doi.org/10.1038/s41592-018-0176-y
